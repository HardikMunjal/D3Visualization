...
app.controller('visualCtrl', function($scope, $http, $sce) {
    ...
    $http.get("http://localhost:4000/parse/")
    ...
    $http.get("http://localhost:4000/json/")
    .then(function(response) {

    var json =response.data;
    console.log('after http',json[0].id);
    console.log('after http',json[0].name);
   ...
   console.log($scope.admin);
    ...
    for(j=0;j<$scope.admin.length;j++){
 
         // console.log($scope.admin[j]);
         //  console.log(json[i]);

        if(JSON.stringify(json[i]) == JSON.stringify($scope.admin[j])){
            json[i].parent=100;
            console.log('done');
        }
    }
    console.log('atlast you fucker',json[0].name)
   }

// console.log(xyz[0].id[0].name);
   var xyz=function getNestedChildren(arr, parent) {
    var out = []
    console.log(arr);
    console.log(parent);
    for(var i in arr) {
    ...
    console.log('aa rha kya bhai tu',out);
    return out
}
var json=xyz(json,100);
...
//                                           {"name": "Naveen", "size": 3938,"children":[
//                                               {"name": "Naveen", "size": 3938 ,"children":[
//                                                   {"name": "Naveen", "size": 3938,"children":[
...
function update(source) {
  ...
  nodeEnter.append("circle")
      ...
      if(d._children && d._children.length>3){
        ...
        .append("svg:title").text(function(d) {return "Time: " + d + 
"\nValue: " + d.name;});
